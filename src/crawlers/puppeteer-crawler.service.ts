...
//   /**
//    * Crawl using Puppeteer. This function lazy-requires puppeteer and related
//    * dependencies so module import doesn't force puppeteer to be installed
//    * when not used (useful for tests).
//    */
//   async crawl(jobId: string, url: string) {
//     // runtime require to avoid import-time errors in tests/environments without puppeteer installed
...
//       // calculate minTime per request (interval / requests)
//       minTime: Math.ceil(
//         config.rateLimitIntervalMs / Math.max(1, config.rateLimitRequests),
//       ),
//     });

//     // wrapper so we respect rate limits
//     const run = (fn: () => Promise<any>) => limiter.schedule(fn);
...
//       } catch (err) {
//         this.logger.warn(
//           'Failed to anonymize proxy, proceeding without proxy',
//           err,
//         );
//         proxiedUrl = null;
//       }
...
//       };
//       if (proxiedUrl) {
//         launchOptions.args.push(`--proxy-server=${proxiedUrl}`);
//       }
...
//         // set UA
//         const ua = this.uaProvider.next() || config.userAgent;
...
//         // goto and wait for page network to quiet down
//         await page.goto(url, {
//           waitUntil: 'networkidle2',
//           timeout: config.jobTimeoutMs,
//         });

//         // check for cancellation right after navigation
//         if (await this.queueService.isCancelled(jobId)) {
...
//         // resolve relative URLs into absolute - do this on server-side
